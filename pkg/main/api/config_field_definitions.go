...
func createSelectFieldDef(name, value string, options []string) FormFieldDefinition {
	// Convert string slice to SelectOption slice
	selectOptions := make([]SelectOption, len(options))
	for i, opt := range options {
		selectOptions[i] = SelectOption{Value: opt, Label: opt}
	}

	return createFormFieldDef(name, fieldTypeSelect, value, selectOptions)
}
...
func (b *OptimizedFieldBuilder) AddSelect(
	name, value string,
	options []string,
) *OptimizedFieldBuilder {
...
func (b *OptimizedFieldBuilder) AddSelectCached(
	name, value, templateType string,
) *OptimizedFieldBuilder {
	b.fields = append(
		b.fields,
		createSelectFieldDef(name, value, getTemplatesWithCache(templateType)),
	)
