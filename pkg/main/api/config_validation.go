...
func validateConfigStructure[T any](configs []T, configType string, validator func(T) error) error {
	...
	for i, config := range configs {
		...
		if err := validator(config); err != nil {
			return fmt.Errorf("%s config %d: %w", configType, i, err)
		}
...
func (cv *ConfigValidator[T]) ValidateAll(configs []T) error {
	...
	if result.Failed > 0 {
		...
		fmt.Fprintf(
			sb,
			"Multiple validation errors (%d failed, %d successful):\n",
			result.Failed,
			result.Successful,
		)
...
func validateInStringList[T any](
	fieldName string,
	validValues []string,
...
func validateNoForbiddenValues[T any](
	fieldName string,
	forbidden []string,
	getValue func(T) []string,
) func(T) error {
