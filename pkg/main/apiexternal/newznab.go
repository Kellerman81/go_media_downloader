...
var errQualityConfig = errors.New("error getting quality config")
...
func processurl(
	...
	for idx := range result {
		if firstid == "" {
			firstid = result[idx].NZB.ID
		}

		results.Add(&result[idx])
	}

	if errors.Is(err, newznab.ErrBroke) {
	...
	// 			case "item":
	// 				b.NZB = apiexternal_v2.Nzb{Indexer: ind, Quality: qual, SourceEndpoint: apiBaseURL}
	// 			case strtitle:
	// 				lastfield = 1
	// 			case strlink:
	// 				lastfield = 2
...
// 			continue
// 		}
// 		var newEntry apiexternal_v2.Nzbwithprio
...
// Uses non-blocking check to avoid waiting during rate limit grace periods.
// Returns true if under limit, false if over limit.
func NewznabCheckLimiter(cfgindexer *config.IndexersConfig) bool {
	c := Getnewznabclient(cfgindexer)
	if c == nil {
		return true
	}

	return c.CheckFreeNonBlocking() == nil
}
...
func QueryNewznabMovieImdb(
	...
	for idx := range qual.Indexer {
		if strings.EqualFold(qual.Indexer[idx].TemplateIndexer, cfgind.Name) {
			additionalQueryParams = qual.Indexer[idx].AdditionalQueryParams
			break
		}
	}
...
func Getnewznabclient(indcfg *config.IndexersConfig) *newznab.Provider {
	provider := providers.GetIndexer(indcfg.Name)
	if provider == nil {
		newNewznab(true, indcfg)
...
func newNewznab(debug bool, idxCfg *config.IndexersConfig) {
	...
	if idxCfg.Limiterseconds == 0 {
		idxCfg.Limiterseconds = 20
	}

	// Create provider with full configuration including custom URLs and rate limits
	provider := newznab.NewProvider(newznab.ProviderConfig{
		IndexerName:       idxCfg.Name,
		BaseURL:           idxCfg.URL,
		APIKey:            idxCfg.Apikey,
		Categories:        []string{}, // Will be set per search from quality profiles
		...
		MaxEntries:        idxCfg.MaxEntries,
		AddQuotesForTitle: idxCfg.Addquotesfortitlequery,
	...
	providers.SetIndexer(idxCfg.Name, provider)
