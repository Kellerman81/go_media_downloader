...
type ListsConfig struct {
	...
	// SkipEmptySize indicates if releases with an empty size are allowed when
	// searching this list's own custom RSS feed (searcher.validateSize's
	// per-list fallback, distinct from QualityIndexerConfig.SkipEmptySize
	// which only applies to real configured indexers)
	SkipEmptySize bool `comment:"Skip releases with no reported file size when searching this list's own feed.\nWhen true, releases without" displayname:"Skip Empty Size Releases (List)" longcomment:"Skip releases with no reported file size when searching this list's own custom feed.\nWhen true, releases without size information are ignored for this list.\nWhen false (default), releases with missing size information are processed normally.\nOnly relevant for list types that search their own feed directly (e.g. custom RSS lists);\nignored for lists that only import a static set of items (e.g. IMDB CSV, Trakt popular).\nDefault: false" toml:"skip_empty_size"`
	...
	// --- IRC announce feed (when listtype = "irc") ---

	// IRCServer is the IRC server address in host:port form (e.g. "irc.tracker.org:6697").
	IRCServer string `comment:"IRC server address as host:port (for type 'irc').\nExample: 'irc.tracker.org:6697'" displayname:"IRC Server" longcomment:"IRC server to connect to for announce-channel monitoring (type 'irc').\nFormat is host:port, e.g. 'irc.tracker.org:6697'.\nUse the TLS port (commonly 6697) together with irc_use_tls = true." toml:"irc_server"`

	// IRCUseTLS enables a TLS connection to the IRC server.
	IRCUseTLS bool `comment:"Use TLS for the IRC connection (for type 'irc').\nEnable for TLS ports such as 6697." displayname:"IRC Use TLS" longcomment:"When true, connect to the IRC server over TLS.\nUse together with a TLS port (commonly 6697).\nDefault: false (plaintext, typically port 6667)." toml:"irc_use_tls"`

	// IRCNick is the nickname to register on the IRC network.
	IRCNick string `comment:"Nickname to use on the IRC network (for type 'irc')." displayname:"IRC Nickname" longcomment:"Nickname to register on the IRC network when monitoring announce channels.\nMust be unique on the network; some trackers require it to match your registered account." toml:"irc_nick"`

	// IRCNickServPassword, when set, sends a NickServ IDENTIFY after connecting.
	IRCNickServPassword string `comment:"NickServ password (for type 'irc').\nWhen set, an IDENTIFY is sent after connecting." displayname:"IRC NickServ Password" longcomment:"Password used to authenticate with NickServ after connecting.\nWhen set, the feed sends 'PRIVMSG NickServ :IDENTIFY <password>'.\nLeave empty if the network/account does not require authentication.\nKeep this value secret." toml:"irc_nickserv_password"`

	// IRCInviteCommand is an optional command sent to a bot to be invited into announce channels.
	IRCInviteCommand string `comment:"Optional invite command sent after auth (for type 'irc').\nExample: 'PRIVMSG Inviter :!invite <user> <key>'" displayname:"IRC Invite Command" longcomment:"Optional raw IRC command sent after authentication to request an invite into the announce channels.\nMany private trackers require messaging an invite bot before they let you JOIN.\nExample: 'PRIVMSG Inviter :!invite myuser mykey'\nLeave empty if no invite step is needed." toml:"irc_invite_command"`

	// IRCChannels are the announce channels to join.
	IRCChannels []string `comment:"Announce channels to join (for type 'irc').\nExample: ['#announce']" displayname:"IRC Channels" longcomment:"List of IRC channels to join and monitor for release announcements.\nInclude the leading '#'.\nExample: ['#announce', '#tracker-announce']" multiline:"true" toml:"irc_channels"`

	// IRCAnnounceNicks restricts which sender nicks are treated as announcements.
	// Empty means accept messages from any nick in the joined channels.
	IRCAnnounceNicks []string `comment:"Sender nicks whose messages are treated as announcements (for type 'irc').\nEmpty = accept all nicks." displayname:"IRC Announce Nicks" longcomment:"Restricts which sender nicknames are parsed as release announcements.\nUsually the tracker's announce bot nick(s).\nLeave empty to accept messages from any nick in the joined channels.\nExample: ['AnnounceBot']" multiline:"true" toml:"irc_announce_nicks"`

	// IRCAnnounceRegex is a Go regexp with named capture groups used to extract release fields.
	// Recognised groups by media type: movie -> imdb; series -> title,tvdb; music -> title,artist;
	// books/audiobooks -> title,author. A 'year' group is optional.
	IRCAnnounceRegex string `comment:"Go regexp with named groups to parse announce lines (for type 'irc').\nGroups: imdb (movies), title+tvdb (series), title+artist (music), title+author (books)." displayname:"IRC Announce Regex" longcomment:"Go regular expression with named capture groups applied to each announce line.\nRecognised groups depend on the media type of the list's parent config:\n  movies            -> 'imdb'  (e.g. tt1234567)\n  series            -> 'title', 'tvdb'\n  music             -> 'title', 'artist'\n  books/audiobooks  -> 'title', 'author'\nAn optional 'year' group is accepted for all types.\nExample: '\\\\[New\\\\]\\\\s+(?P<title>.+?)\\\\s+-\\\\s+(?P<artist>.+?)\\\\s+-'" toml:"irc_announce_regex"`

	// IRCReadSeconds is how long each scheduled run listens to the channels before returning.
	IRCReadSeconds int `comment:"Seconds the first run waits for initial announcements (for type 'irc').\nDefault: 30 when unset." displayname:"IRC Initial Wait Seconds" longcomment:"The IRC connection is kept open in the background and announcements are buffered continuously.\nEach scheduled feed run drains whatever has been buffered since the previous run.\nThis value only applies to the very first run after a session starts: it is how long that run\nwaits for the first announcements to arrive before returning.\nDefault: 30 seconds when 0 or unset." toml:"irc_read_seconds"`
}
...
type NotificationConfig struct {
	...
	// SMTPServer is the SMTP server hostname for sendmail
	SMTPServer string `comment:"SMTP server hostname.\nOnly used when type is 'sendmail'" displayname:"SMTP Server" longcomment:"SMTP server hostname for sending email directly.\nOnly used when type is 'sendmail'.\nExample: 'smtp.gmail.com' or 'mail.example.com'" toml:"smtp_server"`
	// SMTPPort is the SMTP server port for sendmail
	SMTPPort string `comment:"SMTP server port.\nOnly used when type is 'sendmail'" displayname:"SMTP Port" longcomment:"SMTP server port for sending email directly.\nOnly used when type is 'sendmail'.\nLeave empty to use the default (587, STARTTLS submission).\nExample: '587', '465', or '25'" toml:"smtp_port"`
	// SMTPFromEmail is the sender address for sendmail
	SMTPFromEmail string `comment:"Sender email address.\nOnly used when type is 'sendmail'" displayname:"SMTP From Address" longcomment:"Sender email address for outgoing notifications.\nOnly used when type is 'sendmail'.\nExample: 'downloader@example.com'" toml:"smtp_from_email"`
	// SMTPToEmail is the recipient address for sendmail
	SMTPToEmail string `comment:"Recipient email address.\nOnly used when type is 'sendmail'" displayname:"SMTP To Address" longcomment:"Recipient email address for outgoing notifications.\nOnly used when type is 'sendmail'.\nExample: 'you@example.com'" toml:"smtp_to_email"`
	// SMTPUsername is the SMTP auth username for sendmail
	SMTPUsername string `comment:"SMTP authentication username.\nOnly used when type is 'sendmail'" displayname:"SMTP Username" longcomment:"SMTP authentication username.\nOnly used when type is 'sendmail'. Leave empty for an SMTP server that doesn't require authentication.\nExample: 'downloader@example.com'" toml:"smtp_username"`
	// SMTPPassword is the SMTP auth password for sendmail
	SMTPPassword string `comment:"SMTP authentication password.\nOnly used when type is 'sendmail'" displayname:"SMTP Password" longcomment:"SMTP authentication password.\nOnly used when type is 'sendmail'. Leave empty for an SMTP server that doesn't require authentication.\nKeep this value secret and secure." toml:"smtp_password"`
}
