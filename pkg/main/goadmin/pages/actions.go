...
func GetActionsPage(ctx *context.Context) (types.Panel, error) {
	...
	refmovie := action.PopUpWithIframe("refreshmovies", "Refresh Movies Metadata", action.IframeData{Src: "/api/movies/all/refreshall?apikey=" + config.SettingsGeneral.WebAPIKey}, "200px", "200px").ExtContent(ctx)
	refserie := action.PopUpWithIframe("refreshseries", "Refresh Series Metadata", action.IframeData{Src: "/api/series/all/refreshall?apikey=" + config.SettingsGeneral.WebAPIKey}, "200px", "200px").ExtContent(ctx)
	return types.Panel{
