...
func GetDashBoard(cktx *context.Context) (types.Panel, error) {
	 ...
	 * Info Box
	/**************************/
	//components.Label().SetContent("test")
	// cardcard := card.New().
...
func GetDashBoardO(ctx *context.Context) (types.Panel, error) {
...
<textarea class="form-control" id="message-text"></textarea>
	...
	pnl := components.DataTable().SetInfoList(stats).
		...
		SetThead(types.Thead{
			{Head: "Typ", Field: "typ", Sortable: false},
			{Head: "List", Field: "list", Sortable: false},
			{Head: "All", Field: "total", Sortable: false},
			{Head: "Missing", Field: "missing", Sortable: false},
			{Head: "Finished", Field: "finished", Sortable: false},
			{Head: "Upgradable", Field: "upgrade", Sortable: false},
		})
	...
	for _, value := range tasks.GetQueues() {
		queue = append(queue, map[string]types.InfoItem{
			...
			"started": {Content: template.HTML(value.Queue.Started.Format("2006-01-02 15:04:05"))},
		})
	}

	_, _ = row2, row4
	queueitm := components.DataTable().SetInfoList(queue).
		SetThead(types.Thead{
			{Head: "Queue", Field: "queue", Sortable: false},
			{Head: "Added", Field: "added", Sortable: false},
	...
	schedule := components.DataTable().SetInfoList(sched).
		SetThead(types.Thead{
			{Head: "Scheduler", Field: "job", Sortable: false},
			{Head: "Last Run", Field: "lastrun", Sortable: false},
			{Head: "Next Run", Field: "nextrun", Sortable: false},
