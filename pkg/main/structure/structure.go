...
func NewStructure(cfgp *config.MediaTypeConfig, listname string, groupType string, rootpath string, sourcepathstr string, targetpathstr string) (*organizer, error) {
	...
	var listcfg *config.MediaListsConfig
	var cfgqual config.QualityConfig
	if listname != "" {
		listcfg = cfgp.GetList(listname)
		cfgqual = config.Cfg.Quality[listcfg.TemplateQuality]
	}
	return &organizer{
		cfgp:          cfgp,
		listConfig:    listname,
...
func (s *organizer) filterVideoFiles(allfiles *logger.InStringArrayStruct, removesmallfiles bool) error {
	err := scanner.FilterFilesDir(allfiles, s.sourcepathcfg, false, false)
	if err != nil {
		allfiles.Arr = nil
		return err
	}
...
func (s *organizer) removeSmallVideoFile(file string) bool {
	...
	if s.sourcepathcfg.MinVideoSize > 0 && scanner.GetFileSize(file, false) < s.sourcepathcfg.MinVideoSizeByte {
		scanner.RemoveFiles(file, s.sourcepathcfg)
		return true
	}
	return false
}
...
func (s *organizer) fileCleanup(folder string, videofile string) {
	if strings.EqualFold(s.groupType, "movie") || videofile == "" {
		if scanner.RemoveFile(videofile) == nil {
			toRemove, err := scanner.GetFilesDir(folder, s.sourcepathcfg, false)
			if err == nil {
		...
		if scanner.RemoveFile(videofile) == nil {
			for idx := range s.sourcepathcfg.AllowedOtherExtensions {
				scanner.RemoveFile(strings.ReplaceAll(videofile, fileext, s.sourcepathcfg.AllowedOtherExtensions[idx]))
			}
		}
		scanner.CleanUpFolder(folder, s.sourcepathcfg.CleanupsizeMB)
	}
...
func (s *organizer) ParseFileAdditional(videofile string, folder string, deletewronglanguage bool, wantedruntime int, checkruntime bool, m *apiexternal.ParseInfo) error {
	...
	parser.GetPriorityMapQual(m, s.cfgp, s.cfgqual, true, true)
...
func (s *organizer) checkLowerQualTarget(folder string, videofile string, cleanuplowerquality bool, movieid uint, m *apiexternal.ParseInfo) (*logger.InStringArrayStruct, error) {
	...
	var oldpriority int
	if len(moviefiles) >= 1 {
		oldpriority = searcher.GetHighestMoviePriorityByFiles(true, true, movieid, s.cfgp, s.cfgqual)
	...
	oldfiles := &logger.InStringArrayStruct{}
	var entryPrio int
	...
	for idx := range moviefiles {
		...
		entryPrio = searcher.GetMovieDBPriorityByID(true, true, uint(moviefiles[idx].Num), s.cfgp, s.cfgqual)
...
func (s *organizer) GenerateNamingTemplate(videofile string, rootpath string, dbid uint, serietitle string, episodetitle string, mapepi *[]database.DbstaticTwoUint, m *apiexternal.ParseInfo) (string, string) {
	...
	if strings.EqualFold(s.groupType, "movie") {
		...
		movietitle = logger.StringReplaceRune(movietitle, '.', ' ')
		forparser.TitleSource = logger.StringDeleteRuneAll(movietitle, '/')
		...
		forparser.Dbmovie.Title = logger.StringDeleteRuneAll(forparser.Dbmovie.Title, '/')
		...
		forparser.Source.Title = logger.StringDeleteRuneAll(forparser.Source.Title, '/')
		...
		forparser.Dbserie.Seriename = logger.StringDeleteRuneAll(forparser.Dbserie.Seriename, '/')
		if forparser.DbserieEpisode.Title == "" {
			forparser.DbserieEpisode.Title = episodetitle
		}
		forparser.DbserieEpisode.Title = logger.StringDeleteRuneAll(forparser.DbserieEpisode.Title, '/')
		if rootpath != "" {
			foldername, _ = logger.Getrootpath(foldername)
		}
		...
		forparser.TitleSource = logger.StringDeleteRuneAll(serietitle, '/')
		forparser.EpisodeTitleSource = logger.StringDeleteRuneAll(episodetitle, '/')
		if m.Tvdb == "" {
...
func (s *organizer) moveRemoveOldMediaFile(oldfile string, id uint, usebuffer bool, move bool) {
	...
	if strings.EqualFold(s.groupType, "movie") {
		logger.DeleteFromStringsArrCache("movie_files_cached", oldfile)
		...
		logger.DeleteFromStringsArrCache("serie_episode_files_cached", oldfile)
	...
	for idxext := range s.sourcepathcfg.AllowedOtherExtensions {
		...
		if move {
			...
			if scanner.RemoveFile(additionalfile) == nil {
				ok = true
			}
		}
		if ok {
...
func (s *organizer) moveAdditionalFiles(folder string, videotarget string, filename string, videofile string, sourcefileext string, videofilecount int) {
	if strings.EqualFold(s.groupType, "movie") || videofilecount == 1 {
		additionalfiles, err := scanner.GetFilesDir(folder, s.sourcepathcfg, true)
		...
		for idx := range s.sourcepathcfg.AllowedOtherExtensions {
			scanner.MoveFile(strings.ReplaceAll(videofile, sourcefileext, s.sourcepathcfg.AllowedOtherExtensions[idx]), videotarget, filename, &s.sourcepathcfg.AllowedOtherExtensionsIn, &s.sourcepathcfg.AllowedOtherExtensionsNoRenameIn, config.Cfg.General.UseFileBufferCopy, s.targetpathcfg.SetChmod)
		}
	}
}
...
func (s *organizer) organizeSeries(folder string, serieid uint, videofile string, deletewronglanguage bool, checkruntime bool, m *apiexternal.ParseInfo) {
	...
	if s.listConfig != listname || s.listcfg == nil {
		...
		intid := slices.IndexFunc(s.cfgp.Lists, func(c config.MediaListsConfig) bool { return c.Name == m.Listname })
		if intid != -1 {
			s.listcfg = &s.cfgp.Lists[intid]
			s.cfgqual.Close()
			cfgqual := config.Cfg.Quality[s.listcfg.TemplateQuality]
			s.cfgqual = &cfgqual
		}
		// for idx := range s.cfgp.Lists {
		// 	if s.cfgp.Lists[idx].Name == m.Listname {
		// 		s.listcfg = &s.cfgp.Lists[idx]
		// 		break
		// 	}
		// }
	}
	...
	if s.targetpathcfg.MoveReplaced && len(oldfiles.Arr) >= 1 && s.targetpathcfg.MoveReplacedTargetPath != "" {
		for idx := range oldfiles.Arr {
			s.moveRemoveOldMediaFile(oldfiles.Arr[idx], serieid, config.Cfg.General.UseFileBufferCopy, true)
		}
	}
	...
	if m.Priority >= parser.NewCutoffPrio(s.cfgp, s.cfgqual) {
		reached = true
	}
	...
	logger.InsertStringsArrCache("serie_episode_files_cached", videofile)
...
func (s *organizer) organizeMovie(folder string, movieid uint, videofile string, deletewronglanguage bool, checkruntime bool, m *apiexternal.ParseInfo) {
	...
	if s.targetpathcfg.MoveReplaced && oldfiles != nil && len(oldfiles.Arr) >= 1 && s.targetpathcfg.MoveReplacedTargetPath != "" {
		for idx := range oldfiles.Arr {
			s.moveRemoveOldMediaFile(oldfiles.Arr[idx], movieid, config.Cfg.General.UseFileBufferCopy, true)
		}
	}
	if s.targetpathcfg.Usepresort && s.targetpathcfg.PresortFolderPath != "" {
		rootpath = filepath.Join(s.targetpathcfg.PresortFolderPath, foldername)
	...
	logger.InsertStringsArrCache("movie_files_cached", videofile)
	//updatemovie
	targetfile := filepath.Join(videotarget, filename+filepath.Ext(videofile))
...
func (s *organizer) GetEpisodeTitle(firstdbepiid uint, videofile string, m *apiexternal.ParseInfo) (string, string) {
	...
	if serietitle != "" && episodetitle != "" {
		...
		episodetitle = logger.StringReplaceRune(strings.Trim(episodetitle, "."), '.', ' ')

		serietitle = logger.StringReplaceRune(strings.Trim(serietitle, "."), '.', ' ')
	}
...
func (s *organizer) GetSeriesEpisodes(serieid uint, dbserieid uint, videofile string, folder string, m *apiexternal.ParseInfo) (*logger.InStringArrayStruct, bool, []database.DbstaticTwoUint) { //, []int, []database.SerieEpisode, , string, string, int
	...
	if len(episodeArray.Arr) == 1 && m.DbserieEpisodeID != 0 && m.SerieEpisodeID != 0 {
		...
		for idx := range episodeArray.Arr {
			...
			dbserieepisodeid = importfeed.FindDbserieEpisodeByIdentifierOrSeasonEpisode(dbserieid, m.Identifier, m.SeasonStr, episodeArray.Arr[idx])
	...
	for idx := range episodestoimport {
		...
		oldPrio = searcher.GetHighestEpisodePriorityByFiles(true, true, episodestoimport[idx].Num1, s.cfgp, s.cfgqual)
		if m.Priority > oldPrio || oldPrio == 0 {
			for idxfile := range episodefiles {
				if m.Priority > searcher.GetSerieDBPriorityByID(true, true, episodefiles[idxfile], s.cfgp, s.cfgqual) {
			...
			if scanner.RemoveFile(videofile) == nil {
				...
				for idxall := range s.sourcepathcfg.AllowedOtherExtensions {
					scanner.RemoveFile(strings.ReplaceAll(videofile, filepath.Ext(videofile), s.sourcepathcfg.AllowedOtherExtensions[idxall]))
				}
				scanner.CleanUpFolder(folder, s.sourcepathcfg.CleanupsizeMB)
			}
		}
...
func OrganizeSingleFolderAs(folder string, id uint, cfgp *config.MediaTypeConfig, inConfig *Config) {
	...
	videofiles, err := scanner.GetFilesDir(folder, structurevar.sourcepathcfg, false)
	if err != nil {
		return
	}
	...
	if removesmallfiles && structurevar.sourcepathcfg.MinVideoSize > 0 {
		...
		for idx := range videofiles.Arr {
			if scanner.GetFileSize(videofiles.Arr[idx], true) < structurevar.sourcepathcfg.MinVideoSizeByte {
				scanner.RemoveFiles(videofiles.Arr[idx], structurevar.sourcepathcfg)
				removed = true
			}
		}
		if removed {
...
func OrganizeSingleFolder(folder string, cfgp *config.MediaTypeConfig, inConfig *Config) {
	...
	for idxfile := range allfiles.Arr {
		...
		if removefolder {
			...
			for idx := range allfiles.Arr {
				scanner.RemoveFile(allfiles.Arr[idx])
			}
	...
	if structurevar.filterVideoFiles(allfiles, removesmallfiles) != nil {
	...
	for idx := range allfiles.Arr {
		structurevar.organizefileinfolder(allfiles.Arr[idx], deletewronglanguage, checkruntime)
	}
}
...
func (s *organizer) organizefileinfolder(path string, deletewronglanguage bool, checkruntime bool) {
	...
	if s.groupType == "movie" && m.MovieID != 0 && m.DbmovieID != 0 {
		...
		database.QueryStaticStringArray(false,
			...
			&wantedalt)
		if s.cfgqual.Name != templateQuality {
			s.cfgqual.Close()
			cfgqual := config.Cfg.Quality[templateQuality]
			s.cfgqual = &cfgqual
		}

		searchnzb := &apiexternal.Nzbwithprio{WantedTitle: titleyear, WantedAlternates: wantedalt, QualityTemplate: templateQuality, QualityCfg: s.cfgqual, ParseInfo: *m}
