CREATE TABLE `dbmovies` (`id` integer,`created_at` datetime NOT NULL DEFAULT current_timestamp,`updated_at` datetime NOT NULL DEFAULT current_timestamp,`title` text DEFAULT "",`release_date` datetime,`year` integer default 0,`adult` numeric default 0,`budget` integer default 0,`genres` text DEFAULT "",`original_language` text DEFAULT "",`original_title` text DEFAULT "",`overview` text DEFAULT "",`popularity` real default 0,`revenue` integer default 0,`runtime` integer default 0,`spoken_languages` text DEFAULT "",`status` text DEFAULT "",`tagline` text DEFAULT "",`vote_average` real default 0,`vote_count` integer default 0,`moviedb_id` integer default 0,`imdb_id` text DEFAULT "",`freebase_m_id` text DEFAULT "",`freebase_id` text DEFAULT "",`facebook_id` text DEFAULT "",`instagram_id` text DEFAULT "",`twitter_id` text DEFAULT "",`url` text DEFAULT "",`backdrop` text DEFAULT "",`poster` text DEFAULT "",`slug` text DEFAULT "", `trakt_id` integer default 0,PRIMARY KEY (`id`));
...
CREATE TABLE `dbserie_episodes` (`id` integer,`created_at` datetime NOT NULL  DEFAULT current_timestamp,`updated_at` datetime NOT NULL DEFAULT current_timestamp,`episode` text DEFAULT "",`season` text DEFAULT "",`identifier` text DEFAULT "",`title` text DEFAULT "",`first_aired` datetime,`overview` text DEFAULT "",`poster` text DEFAULT "",`dbserie_id` integer default 0,PRIMARY KEY (`id`),CONSTRAINT `fk_dbserie_episodes_dbserie` FOREIGN KEY (`dbserie_id`) REFERENCES `dbseries`(`id`) ON DELETE CASCADE);
