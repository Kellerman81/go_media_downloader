...
func getHighestMoviePriorityByFiles(cfg config.Cfg, movies database.Movie, configEntry config.MediaTypeConfig, quality config.QualityConfig) (minPrio int) {
	...
	if counter >= 1 {
		...
		for idxfile := range foundfiles {
			prio := getMovieDBPriority(cfg, foundfiles[idxfile], configEntry, quality)
			if minPrio == 0 {
				minPrio = prio
			} else {
				if minPrio < prio {
...
func getHighestEpisodePriorityByFiles(cfg config.Cfg, episode database.SerieEpisode, configEntry config.MediaTypeConfig, quality config.QualityConfig) int {
	...
	if counter >= 1 {
		...
		for idxfile := range foundfiles {
			prio := getSerieDBPriority(cfg, foundfiles[idxfile], configEntry, quality)
			if minPrio == 0 {
				minPrio = prio
			} else {
				if minPrio < prio {
